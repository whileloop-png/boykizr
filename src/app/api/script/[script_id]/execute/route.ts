...
const headers_in_use = [
  "boykizr-key",
  "boykizr-rbxlusername",
  "boykizr-rbxlplaceid",
  "boykizr-rbxljobid",
  "boykizr-rbxlgamename",
  "boykizr-executor",
  "boykizr-rbxluserid",
  "boykizr-ismobile"
]
...
async function fetch_script(octokit: Octokit, project_data: any, user_data: any, fingerprint: string) {
  ...
  try {
    ...
    if (response.status !== 200) {
        return new Response(kick_script("guard", "Failed to fetch script from GitHub", is_discord_enabled, discord_link));
    }
...
export async function GET(request: NextRequest, {params}: {params: {script_id: string}}) {
  ...
  if (!fingerprint || fingerprint.trim() == "not found" || fingerprint.trim() == "" ) {
    return new Response(kick_script("guard", "Invalid executor", false, ""));
  }
  ...
  if (check_url_params) {
    ...
    for (const header_key of headers_in_use) {
      if (!validate_param(header_key, JSON.parse(url_params.get("execution_data") ?? "{}"))) {
        return new Response(kick_script("guard", "Invalid request\nMissing: " + header_key, false, ""));
      }
    }

    const data = JSON.parse(url_params.get("execution_data") ?? "{}");
    key = data["boykizr-key"] ?? "";
    username = data["boykizr-rbxlusername"] ?? "username was not found";
    placeid = data["boykizr-rbxlplaceid"] ?? "0";
    jobid = data["boykizr-rbxljobid"] ?? "0";
    gamename = data["boykizr-rbxlgamename"] ?? "Game Name was not found";
    executor = data["boykizr-executor"] ?? "unknown";
    userid = data["boykizr-rbxluserid"] ?? "0";
    is_mobile = data["boykizr-ismobile"] ?? "false";
  }
  ...
  if (!KeyHelper.is_project_valid()) {
    return new Response(kick_script("guard", "Invalid script executed", false, ""));
  }
  ...
  if (banned_users_resp.length > 0) {
    ...
    if (is_perm_ban) {
      ...
      return new Response(kick_script("guard", `You have been permanently blacklisted from this script
  Reason: ${banned_users_resp[0].reason ?? "No reason provided"}`, is_discord_enabled, discord_link));
    ...
    if (is_temp_ban) {
      ...
      if (expires > now) {
        ...
        return new Response(kick_script("guard", `You have been temporarily blacklisted from this script
  ...
  if (project_data.project_type == "paid") {
    const error_script = kick_script("guard", "Invalid key provided", is_discord_enabled, discord_link);
    ...
    if (KeyHelper.get_general_expiration() && key_type != "permanent") {
      if (KeyHelper.is_temp_key_expired()) {
        return new Response(kick_script("guard", "Key has expired", is_discord_enabled, discord_link));
      }
    }
